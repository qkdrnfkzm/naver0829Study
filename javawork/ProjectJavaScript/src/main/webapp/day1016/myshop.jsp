...
$(function(){
	//작은 이미지 클릭시 num 얻어서 myshopdetail.jsp로 이동
...
<body>
	...
	<button type="button" class="btn btn-outline-info btn-sm" onclick="location.href='./myshopinform.jsp'" style="float: right;">상품등록</button>
	</h5>
	</div>
	<table class="table table-bordered shoptb" style="width: 600px">
		...
		<tbody>
			...
			<tr align="center">
				<td><%=++n%></td>
				<td align="left"><img src="../shop/<%=dto.getPhoto() %>" num="<%=dto.getNum()%>" class="small"/><%=dto.getSangpum() %></td>
				<td><div class="box" style="background-color: <%=dto.getColor() %>;"></div></td>
				<td align="right"><%=dto.getPrice() %>원</td>
				<td><%=sdf.format(dto.getWriteday()) %></td>
				<td><button type="button" class="btn btn-sm btn-danger del" num="<%=dto.getNum()%>">삭제</button>
				</td>
			</tr>
			<%} %>
		</tbody>
	...
	<script type="text/javascript">
		$(".del").click(function() {
			let num = $(this).attr("num");
			let a = confirm("삭제하려면 [확인]을 눌러 주세요");
			if (a) {
